...
# НАСТРОЙКИ ТОЛЬКО ДЛЯ SOUNDCLOUD
ydl_opts = {
...
async def play_song(guild_id, url):
    ...
    try:
        ...
        print(f"Общая ошибка воспроизведения: {e}")
        await play_next(guild_id)

@bot.event
async def on_ready():
    print(f'✅ Бот {bot.user} запущен и готов к работе!')
    await bot.change_presence(activity=disnake.Game(name="/play | Маджестик"))

# ==================== МУЗЫКАЛЬНЫЕ КОМАНДЫ ====================

@bot.slash_command(name="play", description="Включить трек с SoundCloud")
async def play(inter: disnake.ApplicationCommandInteraction, search: str):
    ...
    try:
        ...
        await inter.edit_original_response(content=f"❌ Не удалось подключиться к голосовому каналу: {str(e)}")
        return
    ...
    try:
        ...
        def search_soundcloud():
            # Проверяем, является ли search ссылкой на SoundCloud
            ...
            try:
                ...
                print(f"Ошибка поиска на SoundCloud: {e}")
                return None, None
        
        url, title = await loop.run_in_executor(None, search_soundcloud)
        
        if not url:
            await inter.edit_original_response(content="❌ Не удалось найти трек на SoundCloud. Попробуйте другой запрос.")
            return
        ...
        print(f"Критическая ошибка в команде play: {e}")
...
async def botinfo(inter: disnake.ApplicationCommandInteraction):
    ...
    embed.add_field(name="🎵 **Источник музыки:**", value="SoundCloud (стабильная работа)", inline=False)
...
async def commands_list(inter: disnake.ApplicationCommandInteraction):
    ...
    embed.add_field(name="/play [название или ссылка]", value="🎵 Включить трек с SoundCloud", inline=False)
